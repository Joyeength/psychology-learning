...
| F3 | Learning Journal (+ Google Drive auto-sync to Obsidian) | [`learning-journal/`](../learning-journal/) | ux_designer + tech_lead |
...
- **Obsidian plugin riêng** — chỉ dùng vault folder convention (Markdown + .canvas JSON), không build plugin.
- **F3 manual file export** — F3 Obsidian approach là **Google Drive API auto-sync** (revised 2026-05-17), KHÔNG phải manual download `.md`. F4 graph vẫn dùng `.canvas` download (không auto-sync).
- **F3 two-way sync** — chỉ app → Drive (one-way). User edit trong Obsidian sẽ bị overwrite ở sync tiếp.
- **F3 multi-cloud** — chỉ Google Drive, không Dropbox/iCloud/OneDrive trong sprint này.
...
- [x] SPEC F3 → [`../learning-journal/`](../learning-journal/) — complete (01 + 02 + 03-technical-plan done)
...
- [~] BUILD F3 — Journal CRUD + research checklist + Google Drive auto-sync — code complete, pending browser test + QC
